...
void ble_vcp_to_app(uint8_t *data, uint16_t length)
{
...
void on_bleuartServiceEvt(uint16_t connection_handle, ble_uart_evt_t *p_evt)
{
    switch(p_evt->type)
    {
        case BLE_UART_EVT_TX_NOTI_DISABLED:
            rt_kprintf("%02x:bleuart_EVT_TX_NOTI_DISABLED\r\n", connection_handle);
            break;
        case BLE_UART_EVT_TX_NOTI_ENABLED:
            rt_kprintf("%02x:bleuart_EVT_TX_NOTI_ENABLED\r\n", connection_handle);
            break;
                                    ...
                                    uint16_t connSlaveLatency, uint16_t connTimeout)
{
    if(connHandle == peripheralConnList.connHandle)
    {
        ...
        rt_kprintf("Update %x - Int %x \n", connHandle, connInterval);
    }
    else
    {
        rt_kprintf("peripheralParamUpdateCB err..\n");
    }
}
...
static void peripheralStateNotificationCB(gapRole_States_t newState, gapRoleEvent_t *pEvent)
{
    switch(newState & GAPROLE_STATE_ADV_MASK)
    {
        ...
        case GAPROLE_CONNECTED_ADV:
            rt_kprintf("Connected Advertising..\n");
            break;

        case GAPROLE_WAITING:
            if(pEvent->gap.opcode == GAP_END_DISCOVERABLE_DONE_EVENT)
            {
                rt_kprintf("Waiting for advertising..\n");
            }
            else if(pEvent->gap.opcode == GAP_LINK_TERMINATED_EVENT)
            {
                Peripheral_LinkTerminated(pEvent);
                rt_kprintf("Disconnected.. Reason:%x\n", pEvent->linkTerminate.reason);
            }
